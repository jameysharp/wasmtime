...
unsafe extern "C" fn trap_handler(
    ...
    let handled = tls::with(|info| {
        ...
        let details = get_pc_and_fp(context, signum, faulting_addr);
        let test = info.test_if_trap(details, |handler| handler(signum, siginfo, context));
...
unsafe fn get_pc_and_fp(
    ...
    cfg_if::cfg_if! {
        if #[cfg(all(any(target_os = "linux", target_os = "android"), target_arch = "x86_64"))] {
            ...
            let pc = cx.uc_mcontext.gregs[libc::REG_RIP as usize] as *const u8;
            let fp = cx.uc_mcontext.gregs[libc::REG_RBP as usize] as usize;
        } else if #[cfg(all(any(target_os = "linux", target_os = "android"), target_arch = "aarch64"))] {
            let cx = &*(cx as *const libc::ucontext_t);
            let pc = cx.uc_mcontext.pc as *const u8;
            let fp = cx.uc_mcontext.regs[29] as usize;
            ...
            let cx = &*(cx as *const libc::ucontext_t);
            let pc = (cx.uc_mcontext.psw.addr - trap_offset) as *const u8;
            let fp = *(cx.uc_mcontext.gregs[15] as *const usize);
        } else if #[cfg(all(target_os = "macos", target_arch = "x86_64"))] {
            let cx = &*(cx as *const libc::ucontext_t);
            let pc = (*cx.uc_mcontext).__ss.__rip as *const u8;
            let fp = (*cx.uc_mcontext).__ss.__rbp as usize;
            ...
            let cx = &*(cx as *const libc::ucontext_t);
            let pc = cx.uc_mcontext.mc_rip as *const u8;
            let fp = cx.uc_mcontext.mc_rbp as usize;
        } else if #[cfg(all(target_os = "linux", target_arch = "riscv64"))] {
            let cx = &*(cx as *const libc::ucontext_t);
            let pc = cx.uc_mcontext.__gregs[libc::REG_PC] as *const u8;
            let fp = cx.uc_mcontext.__gregs[libc::REG_S0] as usize;
        } else if #[cfg(all(target_os = "freebsd", target_arch = "aarch64"))] {
            let cx = &*(cx as *const libc::mcontext_t);
            let pc = cx.mc_gpregs.gp_elr as *const u8;
            let fp = cx.mc_gpregs.gp_x[29] as usize;
        } else if #[cfg(all(target_os = "openbsd", target_arch = "x86_64"))] {
            let cx = &*(cx as *const libc::ucontext_t);
            let pc = cx.sc_rip as *const u8;
            let fp = cx.sc_rbp as usize;
        }
        else {
