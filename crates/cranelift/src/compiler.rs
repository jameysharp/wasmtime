...
fn save_last_wasm_exit_fp_and_pc(
    ...
    // check to selected functions for how much native stack is remaining.
    // The `VMContext` pointer is the first argument to all functions, and
    // the first field of this structure is `*const VMRuntimeLimits` and the
    // first field of that is the stack limit. Note that the stack limit in
    // this case means "if the stack pointer goes below this, trap". These
    // functions start off by loading the stack limit, checking it against
    // the stack pointer, and optionally trapping.
    //
    ...
    let limits_gv = builder.func.create_global_value(ir::GlobalValueData::Load {
        base: vmctx,
        offset: i32::from(ptr.vmctx_runtime_limits()).into(),
        global_type: pointer_type,
        flags: MemFlags::trusted().with_readonly(),
    });
    let stack_limit = builder.func.create_global_value(ir::GlobalValueData::Load {
        base: limits_gv,
        offset: i32::try_from(ptr.vmruntime_limits_stack_limit())
            .unwrap()
            .into(),
        global_type: pointer_type,
        flags: MemFlags::trusted(),
    });
    builder.func.stack_limit = Some(stack_limit);
