...
fn riscv64_get_operands(inst: &mut Inst, collector: &mut impl OperandVisitor) {
    match inst {
        ...
        Inst::Select {
            ...
            collector.reg_use(&mut condition.rs1);
            collector.reg_use(&mut condition.rs2);
            collector.reg_uses(x.regs_mut());
            collector.reg_uses(y.regs_mut());
            ...
            if dst.regs().len() > 1 {
                ...
                collector.reg_defs(dst.regs_mut());
            }
        }
        Inst::AtomicCas {
            offset,
